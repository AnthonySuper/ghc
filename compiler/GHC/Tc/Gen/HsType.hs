...
{- Note [Wildcard names]
~~~~~~~~~~~~~~~~~~~~~~~~
When emitting wildcard holes we name the unification variable from a
wildcard "w", not "_".  If we use the latter, we get messages like
...
not
   F w1 Int w2 = ...
There is some unsavory magic, relying on that underscore,
...
not when it calls emitNamedWildCardHoleConstraints in
...
we do not call emitAnonWildCardHoleConstraint in tcAnonWildCardOcc.
